...
const { translateTextWithGemini } = require("../gemini/GeminiConfig");

//     if (!selectedLanguage || !data) {
//         return res.status(400).json({ error: 'Both selectedLanguage and data parameters are required' });
//     }
//     console.log("hello world")
//     let translatedText = "Error";
//     if (selectedLanguage === 'unicode') {
//         translatedText = translateToUnicode(data);
//     } else if (selectedLanguage === 'preeti') {
//         translatedText = translateToPreeti(data);
//     } else {
//         translatedText = translateToOtherLanguage(data, selectedLanguage);
//     }

//     // For now, just echo back the input as "translated" text
//     // const translatedText = `[${selectedLanguage}] ${data}`;
//     // res.json({ translatedText });
//     console.log(translatedText)
//     return res.status(200).json({ translatedText: translatedText });
...
exports.translateText = async (req, res) => {
    ...
    try {
        ...
        const translatedText = await translateTextWithGemini(selectedLanguage, data);
...
const translateToPreeti = (text) => {
    // Logic for translating to Preeti will go here
    return `[Preeti] ${text}`;
};

const translateToOtherLanguage = (text, language) => {
    // Logic for other translations will go here
